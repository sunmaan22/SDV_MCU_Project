...
./tx_loop       # 터미널 명령으로 steer/speed/gear 입력 → 패킷 인코딩 + 송신 반복 (Ctrl+C 또는 quit 종료)
```
...
| `input_terminal.hpp` / `input_terminal.cpp` | 별도 스레드에서 stdin 줄 단위 명령(`steer`/`speed`/`gear`/`valid`/`quit`) 파싱 → atomic 상태 갱신. **긴급정지(E-Stop)는 여기 없음** — `DEC-HW-020`에 따라 C 보드 물리 GPIO/EXTI가 처리하는 안전 요구사항이라 RF 링크에 태우지 않는다 | 불필요 |
...
- `tx_loop`: 터미널 명령(`steer`/`speed`/`gear`/`valid`)으로 실시간 값 변경 확인됨. `speed`는 **0~100 크기만**(음수 없음) — 방향은 `gear`(P/R/N/D)가 담당하므로 speed로 또 표현하지 않는다(2026-09-17 결정, `DEC-CTRL-013/019`). 매 전송마다 `MAX_RT/timeout` (모듈 미연결이라 ACK가 올 수 없는 예상된 결과)
