...
> **2026-09-17 E-Stop 전체 삭제:** 데모용 프로젝트라는 이유로 E-Stop 기능을 소프트웨어·하드웨어 전부 삭제했다 (`FINAL_IMPLEMENTATION_SPEC.md` `DEC-HW-020/027`, `DEC-CTRL-006` REMOVED 참고). 아래 §2.1.1(E-Stop 로컬 즉시 차단), §12.3 ISR의 E-Stop 행, §13 Safety의 E-Stop 행 등 E-Stop 전용 절/행은 삭제했다. Gear 물리 입력의 F→C 이전(2026-09-15)은 별개 결정이라 유지한다.
...
>
> **2026-09-15 범위 변경 (2차):** Gear 물리 입력도 F에서 C로 이전했다 (`DEC-HW-026` FROZEN owner node). C는 Gear 상태를 `Driver_Input`에 포함해 CAN 발행한다(당시 함께 이전했던 E-Stop 관련 내용은 2026-09-17에 기능 자체가 삭제됐다). 근거: [`FINAL_IMPLEMENTATION_SPEC.md` §1, §1.2, §3.1, §4.8.1, §8 C Drive](../../system/FINAL_IMPLEMENTATION_SPEC.md).
...
|---|---|---|---|
| v0.5 | 2026-09-17 | C 사용자 | E-Stop 기능 전체 삭제 (소프트웨어·하드웨어, 데모용 프로젝트 근거) |
...
| v0.3 | 2026-09-15 | Team | Gear 물리 입력을 F에서 C로 이전, `Driver_Input`에 포함해 CAN 발행 (당시 함께 이전한 E-Stop은 v0.5에서 삭제) |
...
> Drive + Steering ECU는 RF로 들어오는 기어·조향·속도 요청을 읽어 `Driver_Input`으로 CAN에 발행하고, VCU가 CAN FD로 전달한 최종 속도/조향 명령을 검증해 Motor와 Steering Actuator가 사용할 실제 출력으로 변환하며, 명령값 기반 추정 Speed/RPM과 ECU 상태를 다시 차량 네트워크에 제공한다.

## 1.2 포함 범위

- RF 수신기 기반 조향·속도 요청 읽기 (CAN accel/brake 매핑 OPEN)
- RF Gear 요청 읽기 (P/R/N/D 매핑은 OPEN)
- `Driver_Input` CAN 발행 (C가 publisher; accel/brake/steering/gear 포함)
...
최종 Arbitration은 VCU가 담당한다. 이 ECU는 **Driver/Gear 입력을 읽어 발행하고, VCU가 승인한 최종 명령을 실제 Actuator 제어로 실행하는 Node**다.
...
flowchart TD
    ...
    DI --> DIPUB["Driver_Input CAN TX / accel/brake/steering/gear"]
...
|---|---|---|---|---|---|---|
| OUT-DRV-000 | `Driver_Input` (가속/브레이크/조향/gear) | VCU(F) | CAN FD | TBD (`DEC-CTRL-019` 선형 매핑) | 주기적 | 입력 신호 valid |
...
| REQ-DRV-RF-004 | 재연결만으로 이전 구동 요청을 재적용하지 않는다. 유효한 전체 입력과 확정된 안전 복구 조건을 요구한다. | T-RF-004 |
...
|---|---|---|---|---|---|---|
| `Driver_Input` | TX | C → VCU | TBD (accel/brake/steering/gear) | TBD | N/A | local log if send fail |
...
| `DriverInputTask` | RF(기어·조향·속도 요청) read, `Driver_Input` 산출 | 5~10 ms 후보 / event | High | next control cycle 전 최신화 목표 | 긴 blocking 금지 |
...
IRQ 우선순위는 하드웨어/RTOS 설정 확정 후 검증한다.
