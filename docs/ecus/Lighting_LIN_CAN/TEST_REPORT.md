...
> **2026-09-15 범위 변경:** Ambient Sensor 관련 테스트 항목을 전부 삭제했다. 헤드램프 밝기 제어와 `brake_lamp` 자동 생성(값 중계) 검증 항목으로 대체했다. 근거: [`FINAL_IMPLEMENTATION_SPEC.md` §3.5, §4.6~§4.8, §5.1, §5.2, §8 D Body](../../system/FINAL_IMPLEMENTATION_SPEC.md).
...
| v0.2 | 2026-09-15 | Team | Ambient sensing 시험 항목 삭제, 헤드램프 밝기/brake_lamp 자동 생성 시험 항목으로 대체 |

---

# 1. Test Objective

Gateway의 CAN↔LIN 변환, LIN Master↔Slave 통신, Lighting output(헤드램프 밝기 포함), `brake_lamp` 값 중계, Fault/Timeout 처리와 FreeRTOS timing/queue/stack/watchdog 상태를 검증한다.
...
| Lamp Output | TBD | Driver/LED | load current 확인, 헤드램프 PWM 확인 |
...
| T-BODY-004 | REQ-BODY-004 | 밝기값 0/50/100% 등 단계 입력 | 헤드램프가 on/off가 아닌 밝기로 반응 | NOT RUN | TBD |
...
| T-BODY-015 | REQ-BODY-015 | `Body_Command.brake_lamp` 값 주입 | D가 값을 그대로 LIN으로 중계, 별도 판단 없음 | NOT RUN | TBD |
| T-BODY-016 | REQ-BODY-016 | architecture/code inspect | Ambient Sensor 하드웨어 의존성 없음 | NOT RUN | TBD |
...
|---|---|---|---|---|---|
| T-BODY-001 | Headlamp 밝기 요청 over CAN | Gateway logical request updated | NOT RUN | TBD | TBD |
| T-BODY-002 | Headlamp 밝기 50% 요청 | LIN `LAMP_HEAD_CMD=50%` | NOT RUN | TBD | TBD |
| T-BODY-004 | 밝기 0/50/100% 단계 변화 | lamp 밝기가 단계적으로 변화 | NOT RUN | TBD | TBD |
| T-BODY-005 | LIN Lamp_Command turn/brake ON/OFF | physical lamp toggles | NOT RUN | TBD | TBD |
| T-BODY-007 | valid lamp state | Body_Status CAN reflects state | NOT RUN | TBD | TBD |
| T-BODY-015 | VCU가 감속 감지해 brake_lamp=true 전송 | LIN LAMP_BRAKE_CMD ON, D 자체 판단 없음 | NOT RUN | TBD | TBD |
...
|---|---|---|---|---|---|
| 헤드램프 밝기 최소(0%) | 0% 요청 | 소등 또는 정의된 최소 상태 | NOT RUN | scale TBD | TBD |
| 헤드램프 밝기 최대(100%) | 100% 요청 | 최대 밝기 | NOT RUN | scale TBD | TBD |
| 헤드램프 밝기 out-of-range | >100% 요청 | clamp to 100% | NOT RUN | | TBD |
...
| F-BODY-002 | CAN bus unavailable | controller fault | CAN degraded, LIN task alive | NOT RUN | TBD |
| F-BODY-003 | LIN communication failure | timeout/error | CAN node alive + fault report | NOT RUN | TBD |
| F-BODY-004 | Queue full | send fail/overflow counter | bounded drop/fault policy | NOT RUN | TBD |
| F-BODY-005 | LinScheduleTask delayed | overrun/jitter detect | health degraded/log | NOT RUN | TBD |
| F-BODY-006 | Invalid lamp command | validation fail | output unchanged/safe policy | NOT RUN | TBD |
| F-BODY-007 | headlamp_brightness out-of-range | range validation | clamp to 0-100% | NOT RUN | TBD |
...
| LIN status → CAN Body_Status latency | TBD | NOT RUN | timestamp | TBD |
...
| `Body_Status` | TX | lamp/LIN health (ambient 없음) | NOT RUN | N/A | TBD |
...
|---|---|---|---|---|---|
| `Lamp_Command` | Gateway | lamp command signals (헤드램프 밝기 포함) | NOT RUN | Planned | TBD |
| `Lamp_Status` | Slave | lamp state | NOT RUN | Planned | TBD |
| `Lamp_Diagnostic` | Slave | local fault | NOT RUN | Planned | TBD |

`Ambient_Status` 프레임은 삭제되어 시험 대상이 아니다.
...
```text
[BODY][CAN_RX] Body_Command headlamp_brightness=50 left=0 right=0 brake=0
[BODY][MAP] headlamp_brightness -> LAMP_HEAD_CMD
[LIN][TX] Lamp_Command
[LIN][RX] Lamp_Status head=50
...
- [ ] 헤드램프 밝기 제어 정상
- [ ] `brake_lamp` 값이 D 자체 판단 없이 그대로 중계됨을 확인
...
- LIN bitrate/frame ID/schedule 확정 (Ambient_Status 삭제 반영)
