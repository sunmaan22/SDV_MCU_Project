...
>
> **2026-09-15 범위 변경 (2차):** Gear 물리 입력도 F에서 C로 이전했다. F는 Driver/Gear 입력 GPIO를 갖지 않으며(`LocalInputTask`/`LocalInputAdapter` 제거), C가 `Driver_Input.gear`로 보고한 값을 CAN으로만 받는다. Pi DTC Manager(History DB)는 삭제됐다 — `DTC_Event`는 B(IVI)가 실시간(Active만) 구독·표시하며, F/Pi 어디에도 지속 저장하지 않는다.
>
> **2026-09-17: E-Stop 기능 전체 제거.** 데모 보드 특성상 E-Stop(소프트웨어/하드웨어 전부, 물리 킬스위치 포함)을 프로젝트 전역에서 제거하기로 결정했다. 이 문서의 SafetyTask/SafetyManager/`safety_override`는 이제 E-Stop이 아닌 일반 critical fault(peer timeout 등)만 다룬다. 근거는 [최상위 명세](../../system/FINAL_IMPLEMENTATION_SPEC.md) DEC-HW-020/DEC-HW-027/DEC-CTRL-006(REMOVED) 참고.

[프로젝트 홈](../../../README.md) · [문서 안내](../../README.md) · [폴더 목록](README.md)

> 문서 목적: VCU가 Driver_Input(CAN, gear 포함), CAN Request, Fault를 어떤 FreeRTOS 구조로 받아 최종 차량 명령으로 만드는지 설명한다.
...
포함:
- Driver_Input(CAN) 수신/validation (물리 GPIO 없음 — accel/brake/steering/gear 전부 C가 읽어 CAN으로 발행)
- vehicle mode/state
- arbitration
- safety override
- CAN RX/TX
...
|---:|---|---|
| 1 | Safety | critical fault 발생 시 normal request보다 먼저 safe command 적용 |
...
- F는 물리 GPIO를 갖지 않는다 (Gear 포함 모든 driver 입력은 C 경유)
...
```mermaid
flowchart TD
    DRIVE["Drive ECU"] -->|"Driver_Input / accel/brake/steering/gear"| VCU["VCU"]
...
| `Driver_Input`(gear 포함)은 CAN RX로만 수신, freshness/validation을 CanRxTask에서 처리 | F는 물리 GPIO 없이 C가 발행한 CAN 신호를 신뢰 경계로 검증 |
...
| `InputValidator` | `Driver_Input`(CAN) range/freshness 판단 (accel/brake/steering/gear) |
| `SignalFreshnessManager` | timeout/timestamp 관리 |
| `VehicleStateManager` | P/R/N/D, ready/mode/state 관리 |
| `SafetyManager` | critical fault, override |
...
|---|---|---|---|---|
| `SafetyTask` | critical fault/safety override | event + fast periodic | Highest | blocking 금지 |
| `VcuControlTask` | vehicle state + arbitration + final command | 5~10 ms 후보 | High | logging/CAN blocking 금지 |
| `CanRxTask` | RX decode(`Driver_Input`의 gear 포함), freshness repository update | event | High | short processing |
...
F는 물리 GPIO Task를 갖지 않는다 (2026-09-15부터 `LocalInputTask` 제거, Gear도 C 소유).
...
F는 물리 GPIO ISR(Gear change 등)을 갖지 않는다 — 해당 ISR은 C에 있다.
...
| `SafetyEvent` | Notification/Event Flag | CanRxTask(critical fault 변화)/Health | SafetyTask | critical event 우선 |
...
sequenceDiagram
    ...
    DR0->>RX: Driver_Input (accel/brake/steering/gear)
...
F는 Gear를 포함해 어떤 driver 입력용 물리 GPIO도 갖지 않는다 (2026-09-15부터 전부 C 소유). 실제 CAN transceiver는 TBD.
...
| Signal | Sender | Use |
|---|---|---|
| `Driver_Input` | C | 가속/브레이크/조향/gear (publisher가 C로 이전) |
...
| Data | Owner | Meaning |
|---|---|---|
| `driver_input` | CanRxTask repository | C가 발행한 `Driver_Input`(gear 포함)의 최신 유효값 |
...
| ADR-VCU-006 | Gear GPIO를 F에서 C로 이전, F의 `LocalInputTask` 제거 | Gear 입력을 실제 actuator(C) 옆에서 읽도록 통합 (2026-09-15) |
